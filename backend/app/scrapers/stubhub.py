"""StubHub ticket scraper for F1 events."""
...
class StubHubScraper(BaseScraper):
    ...
    async def scrape_circuit(
        ...
        try:
            ...
            search_url = f"https://www.stubhub.com/search?q={search_query}"
            ...
            async with async_playwright() as p:
                ...
                event_links = await page.query_selector_all(
                    'a[href*="formula-1"], a[href*="grand-prix"], a[href*="f1"]'
                )
                ...
                if event_links:
                    ...
                    if event_url and not event_url.startswith("http"):
                        event_url = f"https://www.stubhub.com{event_url}"
                ...
                if event_url:
                    ...
                    # Parse ticket listing cards
                    ticket_cards = await page.query_selector_all(
                        '[data-testid="listing-card"], .listing-card, '
                        '[class*="TicketCard"], [class*="listing"]'
                    )
                    ...
                    for card in ticket_cards[:30]:
                        try:
                            ...
                            # Extract quantity
                            qty_match = re.search(r"(\d+)\s*ticket", card_text, re.IGNORECASE)
